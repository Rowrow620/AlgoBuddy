...
pub fn generate_daily_temperatures_steps(temperatures: &[i32]) -> Vec<Step> {
    ...
    let all_chars: Vec<char> = temperatures.iter().map(|t| t.to_string().chars().next().unwrap_or(' ')).collect();

    steps.push(Step {
        code_line: 3,
        description: format!("Daily Temperatures for temperatures = {:?}. Monotonic decreasing stack.", temperatures),
    ...
    for (i, &t) in temperatures.iter().enumerate() {
        while let Some(&top_idx) = stack.last() {
            if temperatures[top_idx] < t {
                ...
                let stack_chars: Vec<char> = stack.iter().map(|idx| temperatures[*idx].to_string().chars().next().unwrap_or(' ')).collect();
                steps.push(Step {
                    code_line: 6,
    ...
    steps.push(Step {
        code_line: 9,
        description: format!("Daily Temperatures traversal complete! Output days = {:?}.", res),
        visual: VisualState::Stack {
            chars: all_chars,
            active_idx: None,
            stack: vec![],
