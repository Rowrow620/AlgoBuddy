...
pub fn generate_kth_smallest_bst_steps(tree: &[Option<i32>], k: usize) -> Vec<Step> {
    ...
    steps.push(Step {
        code_line: 10,
        description: format!("Found {}th smallest element = {}.", k, ans),
        visual: VisualState::TreeVisual {
            tree_nodes: nodes,
            active_node_idx: None,
            secondary_node_idx: None,
...
pub fn generate_construct_tree_pre_in_steps(preorder: &[i32], _inorder: &[i32]) -> Vec<Step> {
    let mut steps = Vec::new();
    let mut tree_nodes = Vec::new();
    for &val in preorder {
        tree_nodes.push(Some(val));
    }
...
pub fn generate_tree_max_path_sum_steps(tree: &[Option<i32>]) -> Vec<Step> {
    let mut steps = Vec::new();
    let nodes = tree.to_vec();
    let max_sum = nodes.iter().filter_map(|&n| n).sum::<i32>();
