...
pub fn generate_car_fleet_steps(target: i32, position: &[i32], speed: &[i32]) -> Vec<Step> {
    ...
    let char_repr: Vec<char> = cars.iter().map(|c| c.0.to_string().chars().next().unwrap_or(' ')).collect();

    steps.push(Step {
        code_line: 3,
        description: format!("Car Fleet: target = {}. Sorted cars (pos, time_to_target): {:?}", target, cars),
        visual: VisualState::Stack {
    ...
    for (i, &(pos, time)) in cars.iter().enumerate().rev() {
        ...
        let stack_chars: Vec<char> = stack.iter().map(|t| format!("{:.1}", t).chars().next().unwrap_or(' ')).collect();
        steps.push(Step {
            code_line: 6,
            description: format!("Examining car at pos {} (takes {:.2} hrs to reach target). Push time to stack.", pos, time),
            visual: VisualState::Stack {
                chars: char_repr.clone(),
                active_idx: Some(i),
                stack: stack_chars,
        ...
        if stack.len() >= 2 {
            ...
            if last <= prev {
                ...
                let fleet_chars: Vec<char> = stack.iter().map(|t| format!("{:.1}", t).chars().next().unwrap_or(' ')).collect();
                steps.push(Step {
                    code_line: 8,
    ...
    steps.push(Step {
        code_line: 9,
        description: format!("Car Fleet evaluation complete! Total car fleets reaching target = {}.", fleet_count),
        visual: VisualState::Stack {
            chars: char_repr,
            active_idx: None,
            stack: vec![],
