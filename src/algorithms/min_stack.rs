    ...
    for (op, val) in ops {
        match *op {
            "push" => {
                ...
                steps.push(Step {
                    code_line: 5,
                    description: format!("push({}): main stack = {:?}, minStack = {:?} (current min = {}).", v, stack, min_stack, current_min),
                    visual: VisualState::Stack {
                        chars: vec![],
            ...
            "pop" => {
                if !stack.is_empty() {
                    ...
                    steps.push(Step {
                        code_line: 7,
                        description: format!("pop(): popped {}. main stack = {:?}, minStack = {:?}.", popped, stack, min_stack),
                        visual: VisualState::Stack {
                            chars: vec![],
                            active_idx: None,
                            stack: stack_chars,
