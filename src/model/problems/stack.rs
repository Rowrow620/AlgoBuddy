...
pub fn get_code_lines(problem: Problem, approach_id: usize) -> Option<Vec<(usize, &'static str)>> {
    match (problem, approach_id) {
        ...
        (Problem::GenerateParentheses, _) => Some(vec![
            ...
            (7, "            if openN < n:"),
            (8, "                stack.append(\"(\")"),
            (9, "                backtrack(openN + 1, closedN)"),
            (10, "                stack.pop()"),
            (11, ""),
            (12, "            if closedN < openN:"),
            (13, "                stack.append(\")\")"),
            (14, "                backtrack(openN, closedN + 1)"),
            (15, "                stack.pop()"),
            (16, "        backtrack(0, 0)"),
            (17, "        return res"),
        ]),
        ...
        (Problem::CarFleet, _) => Some(vec![
            ...
            (3, "        pair = sorted(zip(position, speed), reverse=True)"),
            (4, "        stack = []"),
            (5, "        for p, s in pair:"),
            (6, "            stack.append((target - p) / s)"),
            (7, "            if len(stack) >= 2 and stack[-1] <= stack[-2]:"),
            (8, "                stack.pop()"),
            (9, "        return len(stack)"),
        ]),
        (Problem::LargestRectangle, _) => Some(vec![
            ...
            (9, ""),
            (10, "        for i, h in stack: maxArea = max(maxArea, h * (len(heights) - i))"),
            (11, "        return maxArea"),
        ]),
        _ => None,
    }
